...
import { scrapeForm, MCQ_COMPATIBLE_TYPES } from "@/lib/form-scraper";
...
export async function POST(request: NextRequest) {
  ...
  if (!hasSupabase && !hasDb) {
    return NextResponse.json(
      {
        detail:
          "استيراد النماذج يتطلب قاعدة بيانات. أضف Supabase أو Vercel Postgres من إعدادات المشروع، أو استخدم أسئلة القدرات المدمجة.",
      },
      { status: 503 }
    );
  }
  ...
  try {
    ...
    if (!form_url || !skill_category) {
      return NextResponse.json(
        { detail: "رابط النموذج والتصنيف مطلوبان" },
        { status: 400 }
      );
    }
    ...
    // Import to Supabase (preferred)
    if (hasSupabase && supabase) {
      return await importToSupabase(
        supabase,
        formData,
        skill_category,
        difficulty,
    ...
    // Fallback: import to Vercel Postgres
    return await importToVercelPostgres(
      formData,
      skill_category,
      difficulty,
      manualAnswers
    ...
    return NextResponse.json(
      { detail: error.message || "فشل في الاستيراد" },
      { status: 400 }
    );
  }
}
...
async function importToSupabase(
  ...
  formData: Awaited<ReturnType<typeof scrapeForm>>,
  ...
  for (const fq of formData.questions) {
    if (
      fq.question_type === null ||
      !MCQ_COMPATIBLE_TYPES.has(fq.question_type) ||
      fq.choices.length < 2
    ) {
      skipped++;
      continue;
    ...
    const questionText =
      fq.text || (fq.image_url ? "[سؤال مصور]" : "");
    ...
    if (manualIdx !== undefined) {
      correctIndex = manualIdx;
    } else {
      const autoCorrect = fq.choices.findIndex(
        (c) => c.is_correct === true
      );
      if (autoCorrect >= 0) correctIndex = autoCorrect;
    }

    // Build choices array with image URLs
    const choices = fq.choices.map((c) => ({
      text: c.text || "[صورة]",
      image_url: c.image_url,
    }));

    const { error } = await client.from("crawled_questions").insert({
      text: questionText,
      choices: JSON.stringify(choices),
      correct_index: correctIndex >= 0 ? correctIndex : 0,
      section: section,
      difficulty: difficulty,
      source_url: formData.form_url,
      explanation:
        correctIndex >= 0
          ? `الإجابة الصحيحة: ${fq.choices[correctIndex]?.text || ""}`
          : null,
...
async function importToVercelPostgres(
  formData: Awaited<ReturnType<typeof scrapeForm>>,
  skillCategory: string,
  difficulty: string,
  manualAnswers: Record<string, number>
  ...
  if (existing.rows.length > 0) {
    ...
    const ins = await sql`
      ...
      VALUES (${formData.form_url}, ${formData.title}, ${skillCategory}, ${difficulty})
      RETURNING id
    `;
  ...
  for (const fq of formData.questions) {
    ...
    if (ex.rows.length > 0) {
      ...
      imported.push({
        id: ex.rows[0].id,
        content: ex.rows[0].content,
        image_url: ex.rows[0].image_url,
        choices_count: parseInt(cnt.rows[0].c),
      });
      continue;
    }
    ...
    const qr = await sql`
      ...
      VALUES (${content}, ${skillCategory}, ${difficulty}, 45, ${fq.image_url}, ${formImportId}, ${fq.question_id})
      RETURNING id
    `;
    ...
    imported.push({
      ...
      content,
      image_url: fq.image_url,
      choices_count: fq.choices.length,
    });
  }
