"""
Test Engine Router: The CRITICAL PATH.
Must return next question within <100ms.
Analytics and psychometrics are strictly offloaded to background tasks.
If the Shadow Engine fails, the test flow continues flawlessly.
"""
...
from app.analytics.shadow_engine import psychometric_shadow_worker
from app.database import get_db
from app.middleware.nudge_limiter import generate_observational_nudge
from app.models import Question, QuestionChoice, UserAnswer

router = APIRouter(prefix="/api", tags=["test-engine"])
...
@router.get("/question/first", response_model=QuestionOut)
...
@router.post("/answer/submit", response_model=SubmitAnswerResponse)
async def submit_answer(
    ...
    """
    CRITICAL PATH: Must return the next question within <100ms.
    Psychometrics and Graph Pathing are strictly offloaded to background tasks.
    """
    # 1. Validate question exists (fast lookup)
    ...
    chosen_choice = (
        ...
        .filter(QuestionChoice.id == req.chosen_choice_id)
        .first()
    )
    ...
    # 2. Save answer to transactional DB (Fast, synchronous)
    answer = UserAnswer(
    ...
    # 3. Fetch next question via deterministic routing (Fast, no ML)
    ...
    # 4. Check for observational nudge (rate-limited, max 2 per session)
    nudge = None
    try:
        nudge = generate_observational_nudge(
            db, uuid.UUID(req.session_id), uuid.UUID(req.user_id)
        )
    except Exception:
        pass  # Nudge system failure must never block the test

    # 5. Offload heavy psychometrics to background (non-blocking)
    background_tasks.add_task(psychometric_shadow_worker, req.session_id)
